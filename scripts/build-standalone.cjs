...
function buildStandalone(root,output){
 const dist=path.join(root,'dist'),scripts=['i18n.js','parser.js','exporter.js','ipc2581.js','app.js'];let html=fs.readFileSync(path.join(dist,'index.html'),'utf8');
 html=html.replace(/<link rel="stylesheet" href="style\.css(?:\?[^"]*)?">/,'<style>\n'+fs.readFileSync(path.join(dist,'style.css'),'utf8')+'\n</style>');
 for(const file of scripts)html=html.replace(new RegExp('<script src="'+file.replace('.', '\\.')+'(?:\\?[^"]*)?" defer></script>'),'');
